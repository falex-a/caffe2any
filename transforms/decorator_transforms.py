...
def __get_ifm_size(node, tplgy):
    ...
    if node.type in ['Convolution', 'Convolution_ReLU', 'Deconvolution', 'Pooling', 'LRN', 'Softmax']:
        # assert (len(edges) == 1)
        ifm = edges[0].blob
        return ifm.size()
        ...
        return (ifm_shape[1] * ifm_shape[2] * ifm_shape[3]) if ifm_shape else (0)
    else:
        return 0

def __get_weight_size(node, tplgy):
    edges = tplgy.find_incoming_edges(node)
    if node.type in ['Convolution', 'Deconvolution', 'Convolution_ReLU']:
        assert (len(edges) == 1)
        num_ifms = edges[0].blob.shape[1] if edges[0].blob.shape else 0
        if node.type == 'Convolution_ReLU':
            node = node.node1
        return node.kernel_h * node.kernel_w * node.num_output * num_ifms
    elif node.type in ['InnerProduct', 'InnerProduct_ReLU']:
        assert (len(edges) == 1)
...
def __get_MACs(node, ofms_descriptor, tplgy):
    ...
    elif node.type in ['Deconvolution']:
        edges = tplgy.find_incoming_edges(node)
        assert (len(edges) == 1)
        ifms_descriptor = edges[0].blob.shape
        num_ofms = ofms_descriptor[1]
        return node.get_MACs(ifms_descriptor, num_ofms)
    elif node.type in ['InnerProduct', 'InnerProduct_ReLU']:
